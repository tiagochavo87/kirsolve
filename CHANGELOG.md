...
**`inspect` não aceitava diretórios.**
`cmd_run` e `cmd_benchmark` já usavam `load_sources`, mas o `inspect` continuava
chamando `pd.ExcelFile()` direto e falhava com `IsADirectoryError` ao receber
uma pasta. Descoberto ao apontá-lo para a saída real do kir-mapper.

**Tabelas de locus único não eram reconhecidas.**
O kir-mapper grava um arquivo por gene, e o nome do locus está no caminho, não
no cabeçalho:
...
Toda a detecção de layout dependia de encontrar nomes de locus na linha de
cabeçalho, então esses arquivos eram silenciosamente ignorados — pior que um
erro, porque não havia aviso. Agora, quando o cabeçalho não traz locus mas tem
