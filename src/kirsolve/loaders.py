...
def _header_row_por_colunas(df: pd.DataFrame, max_scan: int = 5) -> int | None:
    """Acha o cabecalho por nomes de coluna, quando nao ha locus nele."""
    for i in range(min(max_scan, len(df))):
        celulas = {str(v).strip().lower() for v in df.iloc[i] if pd.notna(v)}
        if celulas & {"sample", "amostra"} and celulas & {"calls", "call", "genotype"}:
            return i
    return None
...
def _read_single_locus_layout(df, header_idx, nome, tool, sample_regex, locus) -> list[Record]:
    """
    Tabela de um unico locus, em que o nome do gene NAO esta no cabecalho e sim
    no caminho do arquivo. E o formato do kir-mapper:

        genotype/cds/KIR3DL3/calls/KIR3DL3.calls.txt
        Sample  Copy_number  Calls  Ratio  Missings

    Sem isso, a deteccao por nome de coluna nao encontra locus nenhum e a
    tabela inteira e ignorada.
    """
    header = [str(v).strip().lower() if pd.notna(v) else "" for v in df.iloc[header_idx]]
    col_amostra = next((j for j, h in enumerate(header)
                        if h in {"sample", "amostra", "id", "sample_id"}), 0)
    col_cn = next((j for j, h in enumerate(header) if "copy" in h), None)
    col_call = next((j for j, h in enumerate(header)
                     if h in {"calls", "call", "alleles", "genotype"}), None)
    if col_call is None:
        return []
    ...
    for i in range(header_idx + 1, len(df)):
        row = df.iloc[i]
        if is_blank(row.iloc[col_amostra]):
            continue
        sample = normalize_sample(row.iloc[col_amostra], sample_regex)
        cns = {}
        if col_cn is not None and not is_blank(row.iloc[col_cn]):
            try:
                cns[canonical_locus(locus)] = int(float(row.iloc[col_cn]))
            except (TypeError, ValueError):
                pass
        out.extend(_make_records(sample, tool or "?", locus,
                                 row.iloc[col_call], cns, nome))
    return out
...
def load_sources(
    ...
    for arq in arquivos:
        for nome, df in _quadros_do_arquivo(arq):
            ...
            # locus vindo do caminho: KIR3DL3/calls/KIR3DL3.calls.txt
            locus_caminho = (find_locus_in_text(arq.stem)
                             or find_locus_in_text(arq.parent.name)
                             or find_locus_in_text(arq.parent.parent.name))
            quadros.append((rotulo, df, ferramenta, locus_caminho))
...
def _processar_quadros(quadros, sample_regex: str) -> pd.DataFrame:
    ...
    for nome, df, tool, locus_caminho in quadros:
        hi = _header_row(df)
        if hi is None:
            # sem locus no cabecalho: pode ser tabela de um locus so, com o
            # gene identificado pelo caminho do arquivo
            hi2 = _header_row_por_colunas(df)
            if hi2 is not None and locus_caminho:
                records.extend(_read_single_locus_layout(
                    df, hi2, nome, tool, sample_regex, locus_caminho))
            continue
